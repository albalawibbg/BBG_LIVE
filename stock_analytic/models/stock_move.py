...
class StockMove(models.Model):
    ...
    #     _logger.debug(f"Starting _action_confirm with merge={merge} merge_into={merge_into}")
    #     try:
    #         result = super(StockMove, self)._action_confirm(merge=merge, merge_into=merge_into)
    #         _logger.debug(f"_action_confirm result: {result}")
    #         return result
    #     except Exception as e:
    #         _logger.error(f"Error in _action_confirm: {e}")
    #         raise
...
class StockMoveLine(models.Model):
    ...
    def update_move_line_analytics(self):
        for line in self:
            if not line.analytic_distribution and line.picking_id.analytic_distribution:
                line.analytic_distribution = line.picking_id.analytic_distribution
